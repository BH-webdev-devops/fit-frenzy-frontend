...
export default function Community() {
    ...
    const [posts, setPosts] = useState<{ title: string, id: number; user_id: number, user_name: string; content: string; created_at: any; replies: { id: number; name: string; comment: string; created_at: any; user_id: number, user_name: string }[]; }[]>([]);
    ...
    const handleReplySubmit = async (e: React.FormEvent, postId: number) => {
        ...
        if (response.ok) {
            ...
            console.error("Error submitting reply:", response.status, response.statusText);
        }
    };
    ...
    const handleDeleteReply = async (postId: number, replyId: number) => {
        const token = localStorage.getItem("token");
        const response = await fetch(`${host}/api/posts/${postId}/reply/${replyId}`, {
            method: "DELETE",
            headers: { Authorization: `${token}` },
        });
        if (response.ok) {
            ...
            console.error("Error deleting reply:", response.status, response.statusText);
        }
    };
    ...
    return (
        <div className="community-container">
            <form onSubmit={handlePostSubmit} className="post-form">
                <textarea value={post} onChange={(e) => setPost(e.target.value)} placeholder="Write a new post..." />
            ...
            <div className="posts-container">
                {posts.map((post) => (
                    <div key={post.id} className="post">
                        ...
                        <button className="small-button" onClick={() => toggleReplies(post.id)}>
                            {visibleReplies[post.id] ? "Hide Replies" : "Show Replies"}
                        </button>
                        {user && post.user_id === user.id && (
                            <button className="small-button delete-button" onClick={() => handleDeletePost(post.id)}>
                            ...
                            <div className="replies-container">
                                {post.replies?.map((reply) => (
                                    <div key={reply.id} className="reply">
                                        ...
                                        {user && reply.user_id === user.id && ( // Check if the current user is the owner of the reply
                                            <button className="small-button delete-button" onClick={() => handleDeleteReply(post.id, reply.id)}>
                                                <FaTrash /> {/* Use the FaTrash icon */}
                                            </button>
                                        )}                                  </div>
                                ))}
